...
class Common_model extends CI_Model
{
    ...
    function del_data_con($table,$con)
    {
        $this->db->where($con);
        $query=$this->db->delete($table);
        if($query)
    ...
    // delete data with condition end


    function chageStatus($value)
    {
       return $this->db->where('id', $value['id'])->update($value['table'], array('status' => $value['status']));
    }
